...
## КРИТИЧЕСКОЕ ПРАВИЛО — WORKFLOW
- **НЕ ИСПОЛЬЗОВАТЬ localhost для проверки!** Проверка ТОЛЬКО на проде.
- Workflow: написать код → `git push` → Render auto-deploy → проверка на https://myheritage.onrender.com
- Пользователь проверяет С ТЕЛЕФОНА на боевом сайте
- Локальный dev-сервер можно использовать ТОЛЬКО для tsc --noEmit, НЕ для визуальной проверки
