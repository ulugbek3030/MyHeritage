export const QuickActions = ({ onCalendar, onExpand, eventCount, expandBadge }: { onCalendar: () => void; onExpand: () => void; eventCount: number; expandBadge?: number | null }) => (
  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,margin:'0 18px 18px'}}>
    {[
      { onClick: onCalendar, icon: '📅', name: 'Календарь', sub: `${eventCount} событий`, badge: eventCount > 0 ? eventCount : null, color: '#60a5fa' },
      { onClick: onExpand, icon: '🌳', name: 'Расширить', sub: 'древо', badge: expandBadge && expandBadge > 0 ? expandBadge : null, color: 'gold' },
    ].map((qa, i) => (
