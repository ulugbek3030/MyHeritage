  ...
  return (
    ...
    >
      ...
      <div ref={content} style={{ position: 'absolute', top: 0, left: 0, width: W, height: H, willChange: 'transform' }}>
        ...
        {/* Second-parent stub: when a child's parents are polygamous (one
            parent has 2+ couples), relatives-tree draws the drop from only
            ONE parent. This adds a visible line from the OTHER parent so
            the child reads as a child of both. Layout is untouched — we
            only add an extra path on top. */}
        {(() => {
          ...
          const lookup = (id: string): { left: number; top: number } | null => {
          ...
          for (const p of visiblePersons) {
            ...
            // Pick the LINEAGE parent (the one whose ancestors stay visible);
            // it's the one NOT in secondaryEntries. relatives-tree usually
            // already drew the connector from the secondary parent.
            ...
            let d: string;
            if (horizSpan < 0.5) {
              // Already in same column — straight drop.
